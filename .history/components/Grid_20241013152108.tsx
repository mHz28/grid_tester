...
const Grid: React.FC = () => {
  ...
  const handleSortSelected = () => {
    ...
    const unselectedData = rowData.filter(
      (row) => !selectedData.includes(row)
    );

    // Sort selected rows based on your desired column (e.g., 'name')
    selectedData.sort((a: RowData, b: RowData) => a.name.localeCompare(b.name));

    // Update the grid data with the sorted rows
